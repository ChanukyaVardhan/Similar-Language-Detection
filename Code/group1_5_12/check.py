...
def main_cal(gram_model):
	...
	# Read the weights and load the model
	m = load_model("result" + str(gram_model) + "_" + str(vecsize));
	label = [];
	dvalues = [];
	with open("input.txt",'r') as ffff:
...
for gram_model in [0, 1, 2, 3, 5]:
	temp = main_cal(gram_model); # Get values of svm for each language from the model
	res.append(deepcopy(temp));
final_labels = [];

# Use Mean Confidence Method for final label
for i in range(0, len(temp)):
	maxval=float("-inf")
	finalLabel=0
...
out_final = open("answers_1_5_12.txt", "w");
